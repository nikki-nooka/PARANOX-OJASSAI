...
export const AuthPage: React.FC<AuthPageProps> = ({ onClose, onAuthSuccess }) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      setError('');
      const result = await signInWithPopup(auth, provider); // Use the imported, initialized auth object
      ...
      if (error.code === 'auth/unauthorized-domain') {
          const requiredDomain = window.location.hostname;
          setError(`DOMAIN NOT AUTHORIZED: Please add '${requiredDomain}' to the authorized domains in your Firebase project's Authentication settings to enable Google Sign-In.`);
      } else {
          setError('Failed to sign in with Google. Please check the console for more details.');
      }
    }
  };
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md lg:max-w-4xl flex flex-col lg:grid lg:grid-cols-2 animate-fade-in-up overflow-hidden max-h-[90vh] lg:max-h-none">
        ...
        <div className="p-8 sm:p-12 relative overflow-y-auto">
            ...
            <div>
              <button onClick={handleGoogleSignIn} className="w-full flex items-center justify-center gap-2 bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 font-semibold py-3 px-4 rounded-md transition-all duration-300 shadow-sm hover:shadow-md">
                  <svg className="w-5 h-5" viewBox="0 0 48 48">
                      ...
                      <path fill="#1976D2" d="M43.611,20.083H42V20H24v8h11.303c-0.792,2.237-2.231,4.166-4.087,5.571l6.19,5.238C42.021,35.591,44,30.035,44,24C44,22.659,43.862,21.35,43.611,20.083z"></path>
                  </svg>
                  Sign in with Google
              </button>
            </div>

            {error && <div className="text-sm text-red-600 bg-red-50 p-3 rounded-md text-center mt-4 font-semibold">{error}</div>}
        </div>
      </div>
    </div>
  );
